...
LegKinematics::LegKinematics(float L1, float L2, float L3){
    this->L1 = L1;
    this->L2 = L2;
    this->L3 = L3;
}

void LegKinematics::setTargetPV(float x, float y, float z){
    p_x = x;
    p_y = y;
    p_z = z;
...
C_Position LegKinematics::fk(float config){
...
J_Position LegKinematics::ik(bool config, float side){
...
J_Position LegKinematics::configCheck(bool config, float side){
...
float LegKinematics::distance(C_Position desired, C_Position calculated){
...
bool LegKinematics::ikCheck(float config){    
